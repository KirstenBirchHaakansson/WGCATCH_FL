...
relative_barplot_year <- function(df, x = year, y = landWt, z = vslFlgCtry, title = title) {
  ...
  df <- mutate(df, x = !!x, y = !!y, z = !!z)
  
  df_sum <- summarise(group_by(df, x, z), y_sum = sum(y, na.rm = T)) 
  ...
  df_cumsum_1 <- mutate(df_cumn_1, z = ifelse(cumn < 12, z, "other"))
  
  df_cumsum_1$z <- as.factor(df_cumsum_1$z)
  
  df_total <- summarise(group_by(df_cumsum_1, x), y_total = sum(y_sum, na.rm = T))
  df_per_x_z <- summarise(group_by(df_cumsum_1, x, z), y_per = sum(y_sum, na.rm = T))
